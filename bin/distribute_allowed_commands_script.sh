...
#    rsync -e "ssh -i /etc/dropbear/id_dropbear_backup" /mnt/OpenWRT_vaults/global_manage/bin/allowed_commands.sh  root@hawking:.ssh
    echo rsync -e \"ssh -p $PORT -i /etc/dropbear/id_dropbear_backup\" -ivv /mnt/OpenWRT_vaults/global_manage/bin/allowed_commands.sh ${USER}@${CLIENT}:.ssh 
done | sort -u | awk '{ print "calling:>>"$0"<<" ; system($0" </dev/null") }'
#done | sort -u | grep hawking | awk '{ print "calling:>>"$0"<<" ; system($0" </dev/null") }'
